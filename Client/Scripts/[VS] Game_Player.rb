...
class Game_Player < Game_Character
  ...
  def usable?(item)
    mp_enough?(item) && sufficient_level?(item) && has_item?(item) &&
    actor.usable?(item) && (item.for_friend? || valid_target?(item.range, item.aoe))
  end
  ...
  def event_trigger_here?(triggers)
    $game_map.events_xy(@x, @y).any? { |event|
      event.trigger_in?(triggers) && !event.normal_priority?
    }
  end
  ...
  def can_attack_range?
    ...
    if collide_with_events?(x, y)
      ...
      return false
    elsif Configs::RANGE_WEAPONS[actor.weapons[0].id][:item_id] > 0 &&
          !$game_party.has_item?($data_items[Configs::RANGE_WEAPONS[actor.weapons[0].id][:item_id]])
      $error_msg = Vocab::NotAmmo
      return false
    elsif Configs::RANGE_WEAPONS[actor.weapons[0].id][:mp_cost] &&
          actor.mp < Configs::RANGE_WEAPONS[actor.weapons[0].id][:mp_cost]
      $error_msg = Vocab::InsufficientMP
      return false
  ...
  def move_by_input
    ...
    # ALTERADO: Input.dir4 → Input.dir8 para suporte a 8 direções (diagonais)
    send_movement(Input.dir8) if Input.dir8 > 0 && !$typing && !$wait_player_move
  end
  ...
  def update_attack
    ...
    $network.send_player_attack if Input.press?(Configs::ATTACK_KEY) &&
      !$game_map.interpreter.running? && can_attack? && !$typing
  end
  ...
  def over_window?
    $windows.each_value do |window|
      if window.is_a?(Window_Chat) && window.opacity > 0 &&
         window.in_area?(0, 0, window.width, window.height + 22)
        return true
      elsif window.is_a?(Window) && window.visible && window.in_area? &&
            !window.is_a?(Window_Chat)
        return true
  ...
  def select_nearest_enemy
    ...
    $game_map.events.each_value do |event|
      ...
      if !blocked_passage?(event) &&
         real_distance_x + real_distance_y < in_sight_sx + in_sight_sy
