...
class Game_Event
  ...
  attr_reader :x
  attr_reader :y
  attr_reader :move_speed, :opacity # FIX: atributos exigidos por send_data.rb
  ...
  def clear_page_settings
    ...
    # FIX: valores padrão para atributos exigidos por send_map_events / send_event_movement
    @move_speed = 4   # velocidade normal (compatível com RPG Maker VX Ace)
    @opacity    = 255 # totalmente opaco
    clear_enemy
    clear_target
  end
  ...
  def setup_page_settings
    ...
    @move_speed = @page.move_speed  # FIX: lê a velocidade configurada na página do evento
  ...
  def conditions_met?(client, page)
    c = page.condition
    if c.switch1_valid
      return false unless client.switches[c.switch1_id] ||
        $network.switches[c.switch1_id] && c.switch1_id > Configs::MAX_PLAYER_SWITCHES
    end
    if c.switch2_valid
      return false unless client.switches[c.switch2_id] ||
        $network.switches[c.switch2_id] && c.switch2_id > Configs::MAX_PLAYER_SWITCHES
    end
    if c.variable_valid
  ...
  def global_conditions_met?(page)
    c = page.condition
    if c.switch1_valid
      return false unless $network.switches[c.switch1_id] &&
        c.switch1_id > Configs::MAX_PLAYER_SWITCHES
    end
    if c.switch2_valid
      return false unless $network.switches[c.switch2_id] &&
        c.switch2_id > Configs::MAX_PLAYER_SWITCHES
    end
    return true
  end
  ...
  def check_event_trigger_touch(x, y)
    ...
    $network.clients.each do |client|
      next unless client&.in_game? || client.map_id == @map_id ||
        client.pos?(x, y) || normal_priority? || !client.event_interpreter.running?
      start(client)
      break
    end
  end
  ...
  def find_target
    target = $network.clients.find { |client|
      client&.in_game? && client.map_id == @map_id && in_range?(client, @sight)
    }
  ...
  #--------------------------------------------------------------------------
  # FIX: Métodos exigidos por send_data.rb (send_map_events / send_event_movement)
  # O servidor não possui animação frame-a-frame; moving é sempre false.
  # max_hp é um alias de mhp definido em Game_Character.
  #--------------------------------------------------------------------------
  def moving
    false
  end
