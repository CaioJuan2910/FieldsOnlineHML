...
#------------------------------------------------------------------------------
# Autor: Valentine
# Modificado: Fields Online Dev Team — integração VS_Logger
#==============================================================================
...
module Handle_Data
  ...
  def handle_messages(client, buffer)
    begin
      ...
      # [LOG] Registra o erro no arquivo de log em vez de apenas exibir no console
      @log.log_error("Erro: #{e}", backtrace: e.backtrace, client: client)
    end
  end
  ...
  def handle_login(client, buffer)
    ...
    # [LOG] Login bem-sucedido — substitui o puts original
    puts("#{user} logou com o IP #{client.ip}.")
    @log.log_login(client)
  end
  ...
  def handle_create_account(client, buffer)
    ...
    # [LOG] Conta criada com sucesso — substitui o puts original
    puts("Conta #{user} criada.")
    @log.log_create_account(client, user)
  end
  ...
  def handle_create_actor(client, buffer)
    ...
    # [LOG] Personagem criado com sucesso
    class_name = $data_classes[class_id]&.name || "Classe##{class_id}"
    @log.log_create_actor(client, name, class_name)
  end
  ...
  def handle_remove_actor(client, buffer)
    ...
    # [LOG] Captura o nome ANTES de remover
    actor_name = client.actors[actor_id].name rescue 'N/A'
    ...
    @log.log_remove_actor(client, actor_name)
  end
  ...
  def handle_use_actor(client, buffer)
    ...
    # [LOG] Jogador entrou no jogo
    @log.log_enter_game(client)
  end
  ...
  def handle_add_drop(client, buffer)
    ...
    # [LOG] Jogador jogou item no chão
    @log.log_player_drop(client, item.name, kind, amount, client.map_id, client.x, client.y)
  end
  ...
  def handle_remove_drop(client, buffer)
    ...
    unless client.full_inventory?(item)
      ...
      # [LOG] Jogador coletou item do chão
      @log.log_player_pickup(client, item.name, drop.kind, drop.amount,
                             client.map_id, drop.x, drop.y,
                             original_owner: drop.name)
    end
  end
  ...
  def handle_create_guild(client, buffer)
    ...
    # [LOG] Guilda criada
    @log.log_guild_create(client, name)
  end
  ...
  def handle_remove_guild_member(client, buffer)
    ...
    if member && @guilds[client.guild_name].leader != member
      ...
      # [LOG] Membro removido da guilda pelo líder
      @log.log_guild_leave(name, client.guild_name, kicked_by: client.name)
    else
  ...
  def handle_leave_guild(client)
    ...
    # [LOG] Captura nome da guilda ANTES de sair
    guild_name_snap = client.guild_name.dup rescue 'N/A'
    member_name     = client.name.dup rescue 'N/A'
    ...
    # [LOG] Jogador saiu/abandonou a guilda
    @log.log_guild_leave(member_name, guild_name_snap)
  end
  ...
  def handle_bank_item(client, buffer)
    ...
    # [LOG] Transação de item no banco (amount positivo = depósito, negativo = retirada)
    @log.log_bank_item(client, item.name, kind, amount)
  end
  ...
  def handle_bank_gold(client, buffer)
    ...
    # [LOG] Transação de ouro no banco (amount positivo = depósito, negativo = retirada)
    @log.log_bank_gold(client, amount)
  end
  ...
  def handle_buy_item(client, buffer)
    ...
    if client.gold >= price * amount && (!client.full_inventory?(item) || amount < 0)
      ...
      # [LOG] Compra em loja
      @log.log_shop_buy(client, item.name, kind + 1, amount, price * amount, client.map_id)
    end
  end
  ...
  def handle_sell_item(client, buffer)
    ...
    if client.item_number(item) >= amount
      ...
      # [LOG] Venda em loja
      @log.log_shop_sell(client, item.name, kind, amount, amount * item.price / 2, client.map_id)
    end
  end
  ...
  def handle_choice_teleport(client, buffer)
    ...
    # [LOG] Captura mapa de origem ANTES de teleportar
    from_map = client.map_id
    dest     = Configs::TELEPORTS[client.teleport_id][index]

    client.transfer(dest[:map_id], dest[:x], dest[:y], Enums::Dir::DOWN)
    client.lose_gold(dest[:gold])

    # [LOG] Teleporte via NPC
    @log.log_teleport(client, from_map, dest[:map_id], dest[:x], dest[:y], gold_cost: dest[:gold])
  end
  ...
  def handle_logout(client)
    # [LOG] Logout voluntário — registra ANTES de sair do jogo
    @log.log_logout(client)
  ...
  def handle_admin_command(client, buffer)
    ...
    # [LOG] Comando administrativo executado
    params_str = "str=#{str} | int1=#{int1} | int2=#{int2} | int3=#{int3}"
    @log.log_admin_command(client, command, params_str)
  end
