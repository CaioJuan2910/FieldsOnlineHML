...
#------------------------------------------------------------------------------
#  Autor: Valentine
#  Revisão: correções de crashes, lógica e segurança
#==============================================================================
...
module Handle_Data
  ...
  def handle_messages(client, buffer)
    begin
      ...
      # safe navigation em backtrace — pode ser nil em alguns contextos de exceção
  ...
  def handle_messages_menu(client, header, buffer)
    ...
    # NOTA: pacotes com header desconhecido também chegam aqui e resetam o timer —
    # comportamento intencional (qualquer pacote válido mantém a conexão ativa).
  ...
  def handle_login(client, buffer)
    ...
    # Evita reprocessamento de login: sem este guard, um cliente já autenticado
    # poderia reenviar o pacote LOGIN e forçar um segundo load_bank + send_login.
    return if client.handshake
  ...
  def handle_create_actor(client, buffer)
    ...
    # Corrigido: a verificação original era 'max_params + points > START_POINTS',
    # que é uma tautologia (sempre falso), pois points = START_POINTS - max_params.
    # O check correto é verificar se o cliente enviou mais pontos do que o permitido.
    return if max_params > Configs::START_POINTS
    # Armazena em variável local para evitar dupla indexação e possível nil
    klass = $data_classes[class_id]
    return unless klass
    return if character_index >= klass.graphics[sex].size
  ...
  def handle_remove_actor(client, buffer)
    ...
    # Armazena referência local para evitar dupla indexação após o delete
    actor = client.actors[actor_id]
    Database.remove_player(actor.id_db)
    client.remove_actor_guild(actor.guild_name, actor.name)
  ...
  def handle_use_actor(client, buffer)
    actor_id = buffer.read_byte
    return unless client.actors.has_key?(actor_id)
    # Verifica se o mapa de destino existe antes de operar sobre ele
    return unless @maps[client.map_id]
    # Define os dados
  ...
  def handle_use_item(client, buffer)
    ...
    # item_id = 0 ou fora do range retorna nil — use_item(nil) causaria crash
  ...
  def handle_use_skill(client, buffer)
    ...
    return if client.using_item?
    # skill_id inválido retorna nil — mesmo problema de use_item
  ...
  def handle_balloon(client, buffer)
    ...
    # Corrigido: balloon_id == 0 passava o guard anterior (só checava > 10)
    return if balloon_id < 1 || balloon_id > 10
    return if client.spawning?
    client.antispam_time = Time.now + 0.5
  ...
  def handle_use_hotbar(client, buffer)
    ...
    # item_id inválido pode retornar nil
    return unless item
    client.use_item(item)
  end
  ...
  def handle_add_drop(client, buffer)
    ...
    # kind/item_id inválidos podem retornar nil — item.soulbound? crasharia
    return unless item
  ...
  def handle_player_param(client, buffer)
    ...
    # Corrigido: == 0 deixava pontos negativos passarem pela guarda
    return if client.points <= 0
    # Corrigido: ponto era decrementado ANTES de validar param_id.
    # Um param_id inválido consumia o ponto permanentemente.
    # Agora validamos primeiro e só decrementamos se o param for reconhecido.
    valid_param = case param_id
                  when Enums::Param::MAXHP, Enums::Param::MAXMP then true
                  when Enums::Param::ATK..Enums::Param::LUK     then true
                  else false
                  end
    return unless valid_param
    client.points -= 1
    case param_id
  ...
  def handle_remove_friend(client, buffer)
    ...
    # Corrigido: sem este guard, online_friends_size poderia ficar negativo
    # se index >= friends.size (delete_at é silencioso, mas o decremento não)
    return if index >= client.friends.size
    client.friends.delete_at(index)
    client.online_friends_size -= 1 if index < client.online_friends_size
  ...
  def handle_remove_guild_member(client, buffer)
    name = buffer.read_string
    return unless client.in_guild? && client.guild_leader?
    # Armazena em variável local para evitar dupla indexação e crash se a guilda
    # for removida por outro contexto entre as duas referências
    guild = @guilds[client.guild_name]
    return unless guild
    member = find_guild_member(guild, name)
    if member && guild.leader != member
  ...
  def handle_bank_item(client, buffer)
    ...
    # kind/item_id inválidos podem retornar nil — soulbound? e full_bank? crashariam
    return unless item
    # amount == 0 é no-op — evita operações desnecessárias
    return if amount == 0
  ...
  def handle_buy_item(client, buffer)
    ...
    # Corrigido: '|| amount < 0' era dead code pois amount = read_short.abs (sempre >= 0)
    if client.gold >= price * amount && !client.full_inventory?(item)
  ...
  def handle_sell_item(client, buffer)
    ...
    return unless client.in_shop?
    return if amount == 0
    # Corrigido: shop_goods[0] sem verificação de existência causava crash se a
    # lista de produtos estivesse vazia
    goods = client.shop_goods
    return unless goods[0]
    return if goods[0][4]
    item = client.item_object(kind, item_id)
    # kind/item_id inválidos podem retornar nil — item.price e item_number crashariam
    return unless item
  ...
  def handle_choice_teleport(client, buffer)
    ...
    # Corrigido: TELEPORTS[teleport_id] sem nil check causava crash se teleport_id
    # fosse inválido ou nil — o acesso subsequente a [index] dispararia NoMethodError
    teleports = Configs::TELEPORTS[client.teleport_id]
    return unless teleports
    # Armazena entry para evitar 3 indexações repetidas
    entry = teleports[index]
    return unless entry
    return if entry[:gold] > client.gold
    client.transfer(entry[:map_id], entry[:x], entry[:y], Enums::Dir::DOWN)
    client.lose_gold(entry[:gold])
  end
  ...
  def handle_trade_item(client, buffer)
    ...
    # Corrigido: 'trade_item_number < amount' com amount negativo NUNCA disparava
    # (comparar um count positivo com um número negativo é sempre false).
    # Corrigido para amount.abs: verifica se há quantidade suficiente para remover.
    return if amount < 0 && client.trade_item_number(container[item_id]) < amount.abs
