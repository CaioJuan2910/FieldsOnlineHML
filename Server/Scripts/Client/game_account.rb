...
#   - receive_data: Buffer_Reader.new → Buffer::Reader.new
#     (buffer.rb agora define Buffer::Reader dentro do module Buffer)
#==============================================================================
module Game_Account
  ...
  def receive_data(data)
    ...
    # CORRIGIDO: Buffer_Reader.new → Buffer::Reader.new
    # buffer.rb agora define Buffer::Reader dentro do module Buffer
    buffer = Buffer::Reader.new(data)
    count  = 0
  ...
  def load_data(actor_id)
    ...
    @guild_name = $network.guilds.has_key?(@actors[actor_id].guild_name) &&
                  $network.member_in_guild?($network.guilds[@actors[actor_id].guild_name], @name) ?
                  @actors[actor_id].guild_name : ''
