...
module Game_Data
  ...
  def load_enemies
    ...
    (1...enemies.size).each do |enemy_id|
      ...
      # Usa helper para evitar repetição; nil se não definido no note
      $data_enemies[enemy_id].ani_index           = read_ani_index(enemies[enemy_id].note)
  ...
  def load_animations
    ...
    (1...animations.size).each do |animation_id|
      ...
      # Carrega frame_max para suportar "Esperar até o fim" no comando Mostrar Animação
  ...
  def load_skills
    ...
    (1...skills.size).each do |skill_id|
      ...
      # Skills e Items usam 8 como padrão (enemies/weapons usam nil)
      $data_skills[skill_id].ani_index          = read_ani_index(skills[skill_id].note, 8)
    end
  end
  ...
  def load_items
    ...
    (1...items.size).each do |item_id|
      ...
      $data_items[item_id].ani_index    = read_ani_index(items[item_id].note, 8)
  ...
  def load_weapons
    ...
    (1...weapons.size).each do |weapon_id|
      ...
      # nil por padrão: weapons sem AniIndex usam a animação padrão do sistema
      $data_weapons[weapon_id].ani_index    = read_ani_index(weapons[weapon_id].note)
  ...
  def load_maps
    ...
    mapinfos.each_key do |map_id|
      begin
        ...
        # Avisa mas continua carregando os demais mapas
        puts("Aviso: Mapa #{map_id} não pôde ser carregado. Erro: #{e}".colorize(:yellow))
      end
    end
  end
  ...
  def load_common_events
    ...
    # Renomeado para common_event_id para não colidir com o array common_events
    (1...common_events.size).each do |common_event_id|
      $data_common_events[common_event_id] = RPG::CommonEvent.new
      $data_common_events[common_event_id].id        = common_events[common_event_id].id
      $data_common_events[common_event_id].trigger   = common_events[common_event_id].trigger
      $data_common_events[common_event_id].switch_id = common_events[common_event_id].switch_id
      $data_common_events[common_event_id].list      = common_events[common_event_id].list
    end
  end
  ...
  def load_motd
    ...
    if File.exist?('motd.txt')
      ...
      # Cria arquivo com mensagem padrão caso não exista
      @motd = 'Bem-vindo ao servidor!'
      File.open('motd.txt', 'w:UTF-8') { |f| f.write(@motd) }
      puts('Arquivo motd.txt não encontrado. Criado com mensagem padrão.'.colorize(:yellow))
    end
  rescue => e
    @motd = ''
    puts("Erro ao carregar motd.txt: #{e}".colorize(:red))
  end
  ...
  def load_global_switches
    ...
    begin
      ...
      puts("Erro ao interpretar switches.json: #{e}".colorize(:red))
      puts('Switches globais não foram carregadas!'.colorize(:red))
    end
  end
  ...
  def load_guilds
    ...
    begin
      ...
      puts("As guildas não foram carregadas! Erro: #{e}".colorize(:red))
    end
  end
  ...
  def save_motd
    # Bloco garante fechamento do arquivo mesmo em caso de erro
    File.open('motd.txt', 'w:UTF-8') { |f| f.write(@motd) }
  rescue => e
    puts("Erro ao salvar motd.txt: #{e}".colorize(:red))
  end

  def save_global_switches
    # 'w:UTF-8' correto para JSON (original usava 'wb' — modo binário inadequado para texto)
    File.open('Data/switches.json', 'w:UTF-8') { |f| f.write(JSON.generate(@switches.data)) }
  rescue => e
    puts("Erro ao salvar switches.json: #{e}".colorize(:red))
  end
  ...
  # Lê o AniIndex de um campo note via regex.
  # Retorna o valor como Integer se encontrado, ou default_value caso contrário.
  # Enemies e Weapons usam nil como padrão; Skills e Items usam 8.
  def read_ani_index(note, default_value = nil)
    value = note[/AniIndex=(.*)/, 1]
