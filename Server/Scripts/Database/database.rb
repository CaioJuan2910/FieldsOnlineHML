...
#
#   [FIX #01 - v1.1] Hash de senha com BCrypt no cadastro de conta.
#
#             PROBLEMA ORIGINAL:
#               :password => pass   # senha em texto plano no banco — crítico!
#
#             Se o banco de dados fosse comprometido (vazamento, acesso não
#             autorizado), todas as senhas estariam expostas diretamente.
#
#             SOLUÇÃO:
#               BCrypt::Password.create(pass)
#
#             Gera um hash seguro com salt aleatório embutido.
#             O hash tem formato: $2a$12$<salt><digest> (~60 chars).
#             A comparação no login usa BCrypt::Password.new(hash) == plain_text,
#             implementada em handle_data.rb.
#
#             IMPORTANTE: Contas existentes com senha em texto plano precisarão
#             ser recadastradas ou migradas manualmente.
#==============================================================================

require 'bcrypt'  # [FIX #01] Gem necessária para hash de senha seguro
...
module Database
  ...
  def self.create_account(user, pass, email)
    ...
    # [FIX #01] Gera o hash bcrypt antes de salvar — senha nunca fica em texto plano
    ...
    s_client[:accounts].insert(
      :username      => user,
      :password      => hashed_pass,   # [FIX #01] hash em vez de texto plano
      :email         => email,
      :vip_time      => Time.now.to_i,
  ...
  def self.load_account(user)
    s_client = sql_client
    result   = s_client[:accounts].where(:username => user)
    #Sequel.ilike(:username, "#{user}%"))

    account          = Account.new
    account.id_db    = result.get(:id)
    account.pass     = result.get(:password)  # retorna o hash — comparação feita em handle_data.rb
  ...
  def self.save_account(client, s_client)
    ...
    vip_time = [
      s_client[:accounts].select(:vip_time).where(:id => client.account_id_db).single_value,
      Time.now.to_i
    ].max

    s_client[:accounts].where(:id => client.account_id_db).update(
      :vip_time => vip_time + client.added_vip_time
    )
  ...
  def self.create_player(client, actor_id, name, character_index, class_id, sex, params, points)
    ...
    (Enums::Param::ATK..Enums::Param::LUK).each { |param_id|
    ...
    s_client[:actors].insert(
      :slot_id        => actor_id,
      :account_id     => client.account_id_db,
      :name           => actor.name,
      ...
      :hp             => actor.hp,
      :mp             => actor.mp,
      :mhp            => actor.param_base[0],
      :mmp            => actor.param_base[1],
      :atk            => actor.param_base[2],
      :def            => actor.param_base[3],
      :int            => actor.param_base[4],
      :res            => actor.param_base[5],
      :agi            => actor.param_base[6],
      :luk            => actor.param_base[7],
      :points         => actor.points,
    ...
    actor.equips.each_with_index { |equip_id, slot_id|
  ...
  def self.load_some_player_data(name)
    ...
    actor = s_client[:actors].select(:account_id, :name).where(:name => name)
    #Sequel.ilike(:name, "#{name}%"))
    s_client.disconnect
  ...
  def self.load_player(row, s_client)
    ...
    actor.self_switches = {}
    s_client[:actor_self_switches].where(:actor_id => actor.id_db).each { |r|
      actor.self_switches[[r[:map_id], r[:event_id], r[:ch]]] = (r[:value] == 1)
    }
  ...
  def self.save_player(client)
    s_client = sql_client
    s_client[:actors].where(:id => client.id_db).update(
      :character_name  => client.character_name,
      :character_index => client.character_index,
      :face_name       => client.face_name,
      :face_index      => client.face_index,
      ...
      :hp              => client.hp,
      :mp              => client.mp,
      :mhp             => client.param_base[0],
      :mmp             => client.param_base[1],
      :atk             => client.param_base[2],
      :def             => client.param_base[3],
      :int             => client.param_base[4],
      :res             => client.param_base[5],
      :agi             => client.param_base[6],
      :luk             => client.param_base[7],
      :points          => client.points,
      :guild_id        => $network.find_guild_id_db(client.guild_name),
      :revive_map_id   => client.revive_map_id,
      :revive_x        => client.revive_x,
      :revive_y        => client.revive_y,
    ...
    client.equips.each_with_index { |equip_id, slot_id|
      s_client[:actor_equips].where(:actor_id => client.id_db, :slot_id => slot_id).update(:equip_id => equip_id)
    }
    client.hotbar.each_with_index { |hotbar, slot_id|
      s_client[:actor_hotbars].where(:actor_id => client.id_db, :slot_id => slot_id).update(:type => hotbar.type, :item_id => hotbar.item_id)
    }
    client.switches.data.each_with_index { |value, switch_id|
      s_client[:actor_switches].where(:actor_id => client.id_db, :switch_id => switch_id + 1).update(:value => value ? 1 : 0)
    }
    client.variables.data.each_with_index { |value, variable_id|
      s_client[:actor_variables].where(:actor_id => client.id_db, :variable_id => variable_id + 1).update(:value => value)
    }
  ...
  def self.save_items(client, actor_items, s_client, item)
    if actor_items.object_id == client.items.object_id   ||
       actor_items.object_id == client.weapons.object_id ||
  ...
  def self.save_player_quests(client, s_client)
    ...
    client.quests.each do |quest_id, quest|
      if quests.include?(quest_id)
        s_client[:actor_quests].where(:actor_id => client.id_db, :quest_id => quest_id).update(
          :state => quest.state, :kills => quest.kills
        )
        ...
        s_client[:actor_quests].insert(
          :actor_id => client.id_db, :quest_id => quest_id, :state => quest.state, :kills => quest.kills
        )
      end
    end
  ...
  def self.save_player_self_switches(client, s_client)
    ...
    client.self_switches.data.each do |key, value|
      if self_switches.has_key?(key) && value != self_switches[key]
        s_client[:actor_self_switches].where(
          :actor_id => client.id_db, :map_id => key[0], :event_id => key[1], :ch => key[2]
        ).update(:value => value ? 1 : 0)
      elsif !self_switches.has_key?(key)
        s_client[:actor_self_switches].insert(
          :actor_id => client.id_db, :map_id => key[0], :event_id => key[1], :ch => key[2], :value => value ? 1 : 0
        )
      end
    end
  end
  ...
  def self.player_exist?(name)
    ...
    player = s_client[:actors].select(:id).where(:name => name)
    #Sequel.ilike(:name, "#{name}%"))
    s_client.disconnect
    !player.empty?
  end
  ...
  def self.create_guild(name)
    s_client = sql_client
    s_client[:guilds].insert(
      :name          => name,
      :leader        => $network.guilds[name].leader,
      :flag          => $network.guilds[name].flag.join(','),
      :creation_date => Time.now.to_i
    )
  ...
  def self.unban(client, player_name)
    s_client       = sql_client
    account_id_db  = s_client[:actors].select(:account_id).where(:name => player_name).single_value
    #Sequel.ilike(:name, "#{player_name}%")).single_value
    s_client.disconnect
